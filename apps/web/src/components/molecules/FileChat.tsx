...
import { MessageResponse, UserDto } from '@notify/types';
...
interface PropType {
  ...
  handleDownloadFile: (bucket: string, folderName: string, chat: MessageResponse) => void;
}

const FileChat = (props: PropType) => {
  const { userIdSession, user, chat, mainRef, handleDeleteChatById, handleDownloadFile } = props;
      ...
      <div className="flex gap-3">
        ...
        {user?.avatar === null && (
          <Avatar className="w-[40px] h-[40px]">
            <AvatarFallback>{user?.name && getSummaryName(user?.name)}</AvatarFallback>
          </Avatar>
        )}
