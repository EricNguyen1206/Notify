...
export class ConversationController {
  ...
  async addUserToConversation(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
  ...
  // Leave conversation
  async leaveConversation(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const userId = (req as any).user.id;
      const conversationId = parseInt(id || "0");
  ...
  async removeUserFromConversation(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const { userId: targetUserId } = req.body;
      const ownerId = (req as any).user.id;
      ...
      await this.conversationService.removeUserFromConversation(ownerId, conversationId, targetUserId);
